...
# 2.2.2 #
#########
def makematrix_1(allw, articlew):
    debug = True
    ...
    awMatrix = {}
    valueCount = 0
    for index, article in enumerate(articlew):
        awMatrix[index] = {}
        valueCount = 0
        for wordAW in trimmedPercent:
            wordTrashhold = wordAW
            if wordAW in articlew[index]:
                awMatrix[index][wordAW] = articlew[index][wordAW]
                valueCount += 1
            else:
                awMatrix[index][wordAW] = 0
